...
const SearchPage = async ({ searchParams: { search } }: Props) => {
  const data = await fetch(
    `${config.apiURL}/api/search?query=${search || ""}`
  );
  ...
  return (
    <div>
      ...
      <div className="max-w-screen-2xl min-h-screen bg-black mx-auto">
        {search && (
          <h3 className="text-4xl text-center text-gray-400 py-10 max-sm:text-3xl">
